"""
ultracathle0.1.1a.py
Ultracathle64 0.1.1a - single-file clean-room N64 HLE/opcode engine shell
...
It is a clean-room starter engine named Ultracathle64.
"""
...
class Ultracathle64Core:
    def __init__(self):
        ...
        self.header = Ultracathle64Header()
        self.rom = bytearray()
        self.rdram = bytearray(8 * 1024 * 1024)
        self.cpu = Ultracathle64CPU(self)
...
class Ultracathle64GUI:
    def __init__(self):
        ...
        self.render_screen("Load an N64 ROM, then boot Ultracathle64 0.1.1a")
    ...
    def build_ui(self):
        ...
        tk.Label(toolbar, text="  Ultracathle64 0.1.1a clean-room N64 core", bg=BG, fg=BLUE, font=("Arial", 11, "bold")).pack(side=tk.LEFT, padx=12)
    ...
    def render_screen(self, message=None):
        ...
        self.screen.create_text(352, 28, text="ULTRACATHLE64 N64 OPCODE ENGINE", fill=GREEN, font=("Consolas", 18, "bold"))
    ...
    def render_info(self, s):
        ...
        text = (
            f"STATUS: {s.get('status')}   BOOT: {s.get('boot')}   CORE: Ultracathle64 0.1.1a clean-room N64 engine\n"
            ...
            f"OPCACHE: {s.get('decode_cache')} entries   HITS: {s.get('decode_cache_hits')}   MISSES: {s.get('decode_cache_misses')}   PROFILE: {s.get('compat_profile')}\n"
            f"FRAME: {s.get('frame')}   VI: {s.get('vi')}   FPS: 60 TRUE/LOCKED   SPEED: {s.get('speed')}   HLE CALLS: {s.get('hle_calls')}   HOOKS: {s.get('hooks')}\n"
            f"NOTE: Ultracathle64 runs a safe subset of N64/MIPS opcodes; many opcodes decode as stubs."
        )
