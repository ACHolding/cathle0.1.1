...
PRIMARY_OPS = {
    ...
    0x28: "SB", 0x29: "SH", 0x2B: "SW", 0x2C: "SDL", 0x2D: "SDR",
...
SPECIAL_OPS = {
    ...
    0x12: "MFLO", 0x13: "MTLO", 0x14: "DSLLV", 0x16: "DSRLV",
    0x17: "DSRAV", 0x18: "MULT", 0x19: "MULTU", 0x1A: "DIV",
    0x1B: "DIVU", 0x1C: "DMULT", 0x1D: "DMULTU", 0x1E: "DDIV",
    0x1F: "DDIVU", 0x20: "ADD", 0x21: "ADDU", 0x22: "SUB",
    0x23: "SUBU", 0x24: "AND", 0x25: "OR", 0x26: "XOR",
    0x27: "NOR", 0x2A: "SLT", 0x2B: "SLTU", 0x2C: "DADD",
    0x2D: "DADDU", 0x2E: "DSUB", 0x2F: "DSUBU", 0x38: "DSLL",
...
class DeviceBus:
    ...
    def reset(self):
        self.regs.clear()
        # Common boot-up register values
    ...
    def handle_mmio(self, addr: int, val: int):
        # High Level emulation of hardware triggers
...
class CPUCore:
    ...
    def execute(self, i: N64Opcode):
        op = PRIMARY_OPS.get(i.op)
        
        if op == "SPECIAL":
            sub = SPECIAL_OPS.get(i.funct)
            if sub == "ADDU": self.gpr[i.rd] = sx32_to_64(u32(self.gpr[i.rs] + self.gpr[i.rt]))
            elif sub == "SUBU": self.gpr[i.rd] = sx32_to_64(u32(self.gpr[i.rs] - self.gpr[i.rt]))
            elif sub == "SLL": self.gpr[i.rd] = sx32_to_64(u32(self.gpr[i.rt] << i.sa))
            elif sub == "SRL": self.gpr[i.rd] = sx32_to_64(u32(self.gpr[i.rt] >> i.sa))
            elif sub == "OR": self.gpr[i.rd] = u64(self.gpr[i.rs] | self.gpr[i.rt])
            elif sub == "AND": self.gpr[i.rd] = u64(self.gpr[i.rs] & self.gpr[i.rt])
            elif sub == "XOR": self.gpr[i.rd] = u64(self.gpr[i.rs] ^ self.gpr[i.rt])
            elif sub == "JR": self.next_pc = u32(self.gpr[i.rs])
            elif sub == "JALR":
                self.gpr[i.rd] = u64(self.pc + 8)
                self.next_pc = u32(self.gpr[i.rs])
            elif sub == "DSLL": self.gpr[i.rd] = u64(self.gpr[i.rt] << i.sa)
            elif sub == "DADDU": self.gpr[i.rd] = u64(self.gpr[i.rs] + self.gpr[i.rt])

        elif op == "REGIMM":
            sub = REGIMM_OPS.get(i.rt)
            if sub == "BGEZ":
                if sign64(self.gpr[i.rs]) >= 0: self.next_pc = u32(self.pc + 4 + (i.simm << 2))
            elif sub == "BLTZ":
                if sign64(self.gpr[i.rs]) < 0: self.next_pc = u32(self.pc + 4 + (i.simm << 2))

        elif op == "LUI": self.gpr[i.rt] = sx32_to_64(i.imm << 16)
        elif op == "ORI": self.gpr[i.rt] = u64(self.gpr[i.rs] | i.imm)
        elif op == "ADDIU": self.gpr[i.rt] = sx32_to_64(u32(self.gpr[i.rs] + i.simm))
        elif op == "LW":
        ...
        elif op == "J": self.next_pc = u32(((self.pc + 4) & 0xF0000000) | (i.target << 2))
        elif op == "JAL":
            self.gpr[31] = u64(self.pc + 8)
            self.next_pc = u32(((self.pc + 4) & 0xF0000000) | (i.target << 2))
        elif op == "BEQ":
            if self.gpr[i.rs] == self.gpr[i.rt]: self.next_pc = u32(self.pc + 4 + (i.simm << 2))
        elif op == "BNE":
            if self.gpr[i.rs] != self.gpr[i.rt]: self.next_pc = u32(self.pc + 4 + (i.simm << 2))
        elif op == "LD":
            addr = u32(self.gpr[i.rs] + i.simm)
            hi = self.core.bus.read_u32(addr)
            lo = self.core.bus.read_u32(addr + 4)
            self.gpr[i.rt] = u64((hi << 32) | lo)
        elif op == "SD":
            addr = u32(self.gpr[i.rs] + i.simm)
            val = self.gpr[i.rt]
            self.core.bus.write_u32(addr, u32(val >> 32))
            self.core.bus.write_u32(addr + 4, u32(val))
        elif op == "COP0":
            if i.rs == 0: # MFC0
                self.gpr[i.rt] = sx32_to_64(self.cp0[i.rd])
            elif i.rs == 4: # MTC0
                self.cp0[i.rd] = u32(self.gpr[i.rt])
