"""
n64hleemu4k0.1.py
N64HLEEmu4K 0.1 - single-file clean-room N64 HLE/opcode engine shell

Python 3.14 compatible.
files = off / one .py file only.
60_fps = true.

Run:
    python3 n64hleemu4k0.1.py

Features:
- Classic emulator-style GUI using tkinter
- Load N64 ROM
- Parse N64 header
- Normalize Z64 / V64 / N64 byte order
- Boot-map ROM into RDRAM
- Black emulator screen
- MIPS III / N64 opcode decoder
- Optimized cached opcode decode path
- Safe CPU execution subset
- HLE/libultra-style hook counters
- Locked 60 FPS scheduler
- PJ64 0.1-style clean-room shell feel

Note:
This does not copy or include Project64, UltraHLE, or commercial emulator source.
It is a clean-room starter engine named N64HLEEmu4K.
"""
...
OPCODE_CACHE_LIMIT = 32768
CLEANROOM_PROFILE = "PJ64_0_1_STYLE_CLEANROOM"
...
MEMORY_PRIMARY_OPS = frozenset({
    ...
    0x30, 0x34, 0x37, 0x38, 0x3C, 0x3F,
})

EXECUTED_SAFE_OPS = frozenset({
    "LUI", "ORI", "ANDI", "XORI", "ADDIU", "ADDI", "DADDIU", "DADDI",
    "ADDU", "ADD", "DADDU", "DADD", "SUBU", "SUB", "DSUBU", "DSUB",
    "AND", "OR", "XOR", "NOR", "SLTI", "SLTIU", "SLT", "SLTU",
    "SLL", "SRL", "SRA", "SLLV", "SRLV", "SRAV", "LW", "SW",
    "J", "JAL", "JR", "JALR", "BEQ", "BNE", "BLEZ", "BGTZ",
    "BLTZ", "BGEZ", "SYSCALL", "BREAK",
})


class N64HLEEmu4KHeader:
    def __init__(self):
        self.valid = False
        self.pi_lat = 0
        self.pi_pwd = 0
...
class N64HLEEmu4KCPU:
    ...
    def decode_name_from_opcode(self, o):
        if o.op == 0:
    ...
    def format_decode(self, word, o=None, name=None):
    ...
    def read_u8(self, vaddr):
        off = self.vaddr_to_rdram(vaddr)
        if 0 <= off < len(self.core.rdram):
            return self.core.rdram[off]
        return 0
    ...
    def vaddr_to_rdram(self, addr):
        addr &= 0xFFFFFFFF
        if 0x80000000 <= addr <= 0x807FFFFF:
            return addr - 0x80000000
        if 0xA0000000 <= addr <= 0xA07FFFFF:
    ...
    def execute(self, word, o=None, name=None):
        if o is None or name is None:
            o, name = self.decode_cached(word)
        ...
        if name == "LUI":
            gpr[o.rt] = u32(o.imm << 16)
        elif name == "ORI":
            gpr[o.rt] = u32(gpr[o.rs] | o.imm)
        elif name == "ANDI":
            gpr[o.rt] = u32(gpr[o.rs] & o.imm)
        elif name == "XORI":
            gpr[o.rt] = u32(gpr[o.rs] ^ o.imm)
        elif name in ("ADDIU", "ADDI", "DADDIU", "DADDI"):
            gpr[o.rt] = u32(gpr[o.rs] + o.simm)
        elif name in ("ADDU", "ADD", "DADDU", "DADD"):
            gpr[o.rd] = u32(gpr[o.rs] + gpr[o.rt])
        elif name in ("SUBU", "SUB", "DSUBU", "DSUB"):
            gpr[o.rd] = u32(gpr[o.rs] - gpr[o.rt])
        elif name == "AND":
            gpr[o.rd] = u32(gpr[o.rs] & gpr[o.rt])
        elif name == "OR":
            ...
            gpr[o.rt] = 1 if s32(gpr[o.rs]) < o.simm else 0
        elif name == "SLTIU":
            gpr[o.rt] = 1 if gpr[o.rs] < (o.simm & 0xFFFFFFFF) else 0
        elif name == "SLT":
            gpr[o.rd] = 1 if s32(gpr[o.rs]) < s32(gpr[o.rt]) else 0
        elif name == "SLTU":
            gpr[o.rd] = 1 if gpr[o.rs] < gpr[o.rt] else 0
        elif name == "SLL":
            gpr[o.rd] = u32(gpr[o.rt] << o.sa)
        elif name == "SRL":
            gpr[o.rd] = u32(gpr[o.rt] >> o.sa)
        elif name == "SRA":
            gpr[o.rd] = u32(s32(gpr[o.rt]) >> o.sa)
        elif name == "SLLV":
            gpr[o.rd] = u32(gpr[o.rt] << (gpr[o.rs] & 0x1F))
        elif name == "SRLV":
            gpr[o.rd] = u32(gpr[o.rt] >> (gpr[o.rs] & 0x1F))
        elif name == "SRAV":
            gpr[o.rd] = u32(s32(gpr[o.rt]) >> (gpr[o.rs] & 0x1F))
        elif name == "LW":
            gpr[o.rt] = self.read_u32(u32(gpr[o.rs] + o.simm))
        elif name == "SW":
            self.write_u32(u32(gpr[o.rs] + o.simm), gpr[o.rt])
        elif name == "J":
            ...
            self.next_pc = u32(gpr[o.rs])
        elif name == "JALR":
            gpr[o.rd or 31] = u32(old_pc + 8)
            self.next_pc = u32(gpr[o.rs])
        elif name == "BEQ":
            if gpr[o.rs] == gpr[o.rt]:
...
class N64HLEEmu4KCore:
    def __init__(self):
        ...
        self.header = N64HLEEmu4KHeader()
        self.rom = bytearray()
        self.rdram = bytearray(8 * 1024 * 1024)
        self.cpu = N64HLEEmu4KCPU(self)
    ...
    def tick_frame(self):
        ...
        steps = 160 if self.ultra_speed else 32
...
class N64HLEEmu4KGUI:
    """Compact 600x400 Tkinter shell for the clean-room N64 HLE core."""
    ...
    def build_ui(self):
        ...
        tk.Label(
            toolbar,
            text=" N64HLEEmu4K 0.1 | py3.14 | files off",
            bg=BG,
            fg=BLUE,
            font=("Arial", 8, "bold"),
    ...
    def draw_center_text(self, y, text, fill=GREEN, size=9, bold=True):
    ...
    def render_screen(self, message=None):
        ...
        frame = s["frame"]
        title = (s.get("title") or s["rom"] or "UNKNOWN")[:40]
        pulse = 10 + (frame % 30)
        x = 40 + int((w - 100) * ((frame % 120) / 120.0))

        self.draw_center_text(16, "N64HLEEmu4K OPCODE/HLE ENGINE", GREEN, 10)
        self.draw_center_text(38, title, WHITE, 9)
        self.draw_center_text(60, f"PC {s['pc']:08X}  OP {s['last_opcode']:08X}", GREEN, 8)
        self.draw_center_text(80, s["last_decode"][:52], WHITE, 8, False)
        ...
        self.draw_center_text(
            h - 12,
            f"F{s['frame']} VI{s['vi']} OPS{s['opcode_count']} HLE{s['hle_calls']} {s['speed']}",
            GREEN,
            8,
        )
    ...
    def load_rom(self):
        ...
        try:
            ...
            self.rom_list.insert(tk.END, (info.get("title") or "UNKNOWN TITLE")[:22])
    ...
    def render_info(self, s):
        ...
        decode = (s.get("last_decode") or "---")[:42]
        title = (s.get("title") or "---")[:28]
        text = (
            f"{s.get('status')} | {s.get('boot')[:32]}\n"
            f"ROM {s.get('rom')[:24]} | {size_mb:.2f} MB | {s.get('rom_type')[:18]}\n"
            f"TITLE {title} | ID {s.get('cart_id')} {s.get('country')} v{s.get('version')}\n"
            f"PC {s.get('pc',0):08X} NEXT {s.get('next_pc',0):08X} BOOT {s.get('boot_pc',0):08X}\n"
            f"CRC {s.get('crc1',0):08X}/{s.get('crc2',0):08X} | {s.get('cic')[:18]}\n"
            f"OP {s.get('last_opcode',0):08X} | {decode}\n"
            f"OPS {s.get('opcode_count')} CACHE {s.get('decode_cache')} HLE {s.get('hle_calls')} FPS 60"
        )
