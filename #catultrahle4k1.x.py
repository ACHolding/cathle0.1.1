...
- RSP & RDP HLE: High-level task processing and Display List parsing.
- Audio Interface (AI): DMA consumption and timing simulation.
- Serial Interface (SI): PIF RAM processing and controller input handling.
- Translation Lookaside Buffer (TLB): Virtual-to-physical memory mapping.
- Commercial ROM Booter: IPL3 RAM loading and Header parsing.
...
class DeviceBus:
    ...
    def write_u32(self, addr: int, val: int):
        ...
        elif 0x04001000 <= p_addr < 0x04002000:
...
class CPUCore:
    """mew64 R4300i Full Instruction Set Interpreter with TLB."""
    def __init__(self, core: ACsN64Core):
        self.core = core
        self.gpr = [0] * 32
        self.fpr = [0] * 32
    ...
    def execute(self, o: N64Opcode):
        ...
        # Coprocessor 0 & TLB
            ...
            idx = random.randint(self.cp0[CP0_WIRED] & 0x1F, 31)
            self._write_tlb_entry(idx)
        elif name == "TLBP":
            hi = self.cp0[CP0_ENTRYHI]
            vpn2 = (hi >> 13) & 0x7FFFF
...
class ACsN64Core:
    ...
    def boot(self) -> bool:
        """Boot process simulating CIC/PIF IPL3 loader to start commercial ROMs."""
        ...
        # Simulate IPL3 boot process: Copy up to 1MB of ROM to RDRAM
        boot_len = min(len(self.rom), 0x100000)
        self.rdram[0:boot_len] = self.rom[0:boot_len]
        
        # Older game compatibility window
        rom_window = min(0x200000, len(self.rom), len(self.rdram) - 0x100000)
        if rom_window > 0:
            self.rdram[0x100000:0x100000 + rom_window] = self.rom[0:rom_window]
            
        # Initialize CPU state post-IPL3
        self.cpu.pc = self.header.boot_address if self.header.boot_address else 0x80000400
    ...
    def trigger_si_dma(self, read_pif: bool):
        ...
        if read_pif: # PIF RAM -> RDRAM
            ...
            self.rdram[dram_addr:dram_addr+64] = self.pif_ram[0:64]
        else: # RDRAM -> PIF RAM
            self.pif_ram[0:64] = self.rdram[dram_addr:dram_addr+64]
