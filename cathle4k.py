...
"""
n64emuhle4k0.1.py
N64EmuHLE4K 0.1 - single-file clean-room N64 emulator core + Tkinter GUI + Corn-style HLE profiles.
...
Run:
    python3.14 n64emuhle4k0.1.py

Optional Cython translation outside the app, still using this one source file:
    python3.14 -m pip install cython
    python3.14 -m cython -3 --module-name n64emuhle4k01 n64emuhle4k0.1.py -o n64emuhle4k01.c
...
- Corn-style no-plugin HLE fast path with a PJ64 0.1-style classic GUI feel.

This file intentionally does not copy Corn, Project64, UltraHLE, Nintendo, or
commercial emulator source. It is a clean-room, educational emulator-core scaffold.
"""
...
APP_NAME = "ac's N64EmuHLE4K 0.1"
ENGINE_FILE = "n64emuhle4k0.1.py"
...
CLEANROOM_PROFILE = "CORN_0_3_STYLE_CLEANROOM_FAST_HLE4K_NO_PLUGIN_CORE"
...
class ChatGPTN64EmuGUI:
    ...
    def build_ui(self) -> None:
        ...
        tk.Label(
            toolbar,
            text=" N64EmuHLE4K 0.1 | python3.14 | Corn-style HLE | files off",
            bg=BG,
            fg=BLUE,
            font=("Arial", 8, "bold"),
    ...
    def render_mario_kart_hle(self, s: Dict[str, object]) -> None:
        ...
        self.draw_center_text(15, "N64EmuHLE4K 0.1", GREEN, 9)
        if progress < 40:
...
def _corn_render_corn_hle(self: "ChatGPTN64EmuGUI", s: Dict[str, object]) -> None:
    ...
    if short == "SM64":
        ...
        self.screen.create_oval(kart_x + 2, 125, kart_x + 10, 133, fill=BLACK, outline=BLACK)
        self.screen.create_oval(kart_x + 20, 125, kart_x + 28, 133, fill=BLACK, outline=BLACK)
    self.draw_center_text(14, "N64EmuHLE4K 0.1", GREEN, 9)
...
def _selftest() -> None:
    core = ChatGPTN64EmuCore()
    info = core.load_rom_bytes(_make_synthetic_sm64_rom(), "Super Mario 64_test.z64")
    assert info["rom_type"] == "Z64 BIG-ENDIAN", info
    assert info["hle_game"] == SUPER_MARIO_64_PROFILE, info
    assert info["corn_source_imported"] is False, info
    info = core.boot()
    assert info["booted"] is True, info
    assert "CORN-HLE4K" in str(info["boot"]), info
    for _ in range(20):
        info = core.tick_frame()
    assert int(info["frame"]) == 20, info
    assert int(info["corn_recompiled_blocks"]) > 1, info
    assert str(info["hle_video_mode"]).startswith("SM64"), info
    assert int(info["hle4k_target_width"]) == 3840, info
...
if __name__ == "__main__":
    if "--selftest" in sys.argv:
        _selftest()
        print("n64emuhle4k0.1 selftest ok")
    else:
        main()
