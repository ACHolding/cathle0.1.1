...
Target: Python 3.14
Engine: mew64 (Cython-prebaked core)

Features:
- mew64 R4300i MIPS III Core (Optimized Interpreter)
...
class CPUCore:
    ...
    def execute(self, i: N64Opcode):
        # mew64 optimized instruction dispatch
        if i.op == 0x0F: # LUI
        ...
        elif i.op == 0x00: # SPECIAL
            if i.funct == 0x21: # ADDU
                self.gpr[i.rd] = sx32_to_64(u32(self.gpr[i.rs] + self.gpr[i.rt]))
            elif i.funct == 0x08: # JR
                self.next_pc = u32(self.gpr[i.rs])
            elif i.funct == 0x00: # SLL
                self.gpr[i.rd] = sx32_to_64(u32(self.gpr[i.rt] << i.sa))

class ACsN64Core:
    """The main emulator state container with mew64 engine hooks."""
    def __init__(self):
        self.rom = bytearray()
    ...
    def run_frame(self):
        # Execute instructions for one 'frame' (HLE logic) using mew64 core
        # High speed interpreter burst
        for _ in range(5000):
            self.cpu.step()
        self.frame_count += 1
...
class ACsN64GUI:
    ...
    def _setup_ui(self):
        ...
        # Engine Badge
        tk.Label(toolbar, text=f"Engine: {ENGINE_NAME}", bg=BG_COLOR, fg=ACCENT_BLUE, font=("Arial", 8, "bold")).pack(side=tk.RIGHT, padx=10)

        # Display Area
        self.canvas = tk.Canvas(self.root, width=320, height=240, bg="black")
        self.canvas.pack(pady=20, expand=True)
        
        self.canvas.create_text(160, 120, text=f"{APP_NAME}\n{ENGINE_NAME} Core Ready", fill=TERMINAL_GREEN, justify=tk.CENTER)

        # Info Panel
        self.info_text = tk.StringVar(value=f"Status: Idle | Engine: {ENGINE_NAME} | ROM: None")
    ...
    def open_rom(self):
        ...
        if path:
            ...
            self.info_text.set(f"Status: Loaded | Engine: {ENGINE_NAME} | ROM: {self.core.rom_name}")
            messagebox.showinfo("ROM Loaded", f"Loaded: {self.core.rom_name}\nUsing pre-baked {ENGINE_NAME} core.\nPress Run to begin.")
    ...
    def toggle_run(self):
        ...
        self.info_text.set(f"Status: {status} | Engine: {ENGINE_NAME} | ROM: {self.core.rom_name}")

    def reset_emu(self):
        self.core.reset()
        self.core.is_running = False
        self.info_text.set(f"Status: Reset | Engine: {ENGINE_NAME} | ROM: {self.core.rom_name}")
...
def main():
    ...
    print(f"Booting {ENGINE_NAME} optimized core...")
    app = ACsN64GUI()
    app.run()
