...
"""
cathle0.1.1b.py
CatHLE64 0.1.1b - single-file clean-room N64 emulator core + Tkinter GUI + Corn public-feature HLE/recompiler layer.
...
commercial emulator source. It is a clean-room, educational emulator-core scaffold with a CatHLE64/Corn-style HLE boot path.
"""
...
CLEANROOM_PROFILE = "CATHLE640_1_CORN_PUBLIC_FEATURES_CLEANROOM_HLE"
...
class ChatGPTN64EmuGUI:
    ...
    def build_ui(self) -> None:
        ...
        tk.Label(
            toolbar,
            text=" n64emuv0.1 | cathle640.1 | Corn public-feature HLE | files off",
            bg=BG,
            fg=BLUE,
            font=("Arial", 8, "bold"),
    ...
    def render_mario_kart_hle(self, s: Dict[str, object]) -> None:
        ...
        self.draw_center_text(15, "n64emuv0.1", GREEN, 9)
        if progress < 40:
...
def _corn_profile_for(core: "ChatGPTN64EmuCore") -> Optional[Dict[str, object]]:
    profile = CORN_HLE_PROFILES.get(getattr(core, "hle_game", GENERIC_PROFILE))
    return profile


def _corn_ensure_attrs(core: "ChatGPTN64EmuCore") -> None:
...
_original_configure_hle_profile = ChatGPTN64EmuCore.configure_hle_profile

def _corn_configure_hle_profile(self: "ChatGPTN64EmuCore") -> None:
...
ChatGPTN64EmuCore.configure_hle_profile = _corn_configure_hle_profile


_original_load_rom_bytes = ChatGPTN64EmuCore.load_rom_bytes

def _corn_load_rom_bytes(self: "ChatGPTN64EmuCore", raw: bytearray | bytes, name: str = "memory.rom", path: str = "") -> Dict[str, object]:
...
ChatGPTN64EmuCore.load_rom_bytes = _corn_load_rom_bytes


_original_reset = ChatGPTN64EmuCore.reset

def _corn_reset(self: "ChatGPTN64EmuCore") -> None:
...
ChatGPTN64EmuCore.reset = _corn_reset


_original_boot = ChatGPTN64EmuCore.boot

def _corn_boot(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
...
ChatGPTN64EmuCore.boot = _corn_boot


def _corn_tick_hle(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    profile = _corn_profile_for(self)
    if profile is None:
        return self.info()
    _corn_ensure_attrs(self)
...
ChatGPTN64EmuCore.tick_corn_hle = _corn_tick_hle


_original_tick_frame = ChatGPTN64EmuCore.tick_frame

def _corn_tick_frame(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    if not self.booted:
        return self.info()
...
ChatGPTN64EmuCore.tick_frame = _corn_tick_frame


_original_run_steps = ChatGPTN64EmuCore.run_steps

def _corn_run_steps(self: "ChatGPTN64EmuCore", count: int) -> Dict[str, object]:
...
ChatGPTN64EmuCore.run_steps = _corn_run_steps


_original_info = ChatGPTN64EmuCore.info

def _corn_info(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    _corn_ensure_attrs(self)
...
ChatGPTN64EmuCore.info = _corn_info


_original_render_screen = ChatGPTN64EmuGUI.render_screen

def _corn_render_screen(self: "ChatGPTN64EmuGUI", message: Optional[str] = None) -> None:
    if message is None:
        s = self.core.info()
...
ChatGPTN64EmuGUI.render_screen = _corn_render_screen


def _corn_render_corn_hle(self: "ChatGPTN64EmuGUI", s: Dict[str, object]) -> None:
    w, h = self.screen_width, self.screen_height
    frame = int(s.get("frame", 0) or 0)
    progress = int(s.get("hle_boot_progress", 0) or 0)
    ...
    if short == "SM64":
        ...
        self.screen.create_oval(kart_x + 2, 125, kart_x + 10, 133, fill=BLACK, outline=BLACK)
        self.screen.create_oval(kart_x + 20, 125, kart_x + 28, 133, fill=BLACK, outline=BLACK)
    self.draw_center_text(14, "n64emuv0.1", GREEN, 9)
...
ChatGPTN64EmuGUI.render_corn_hle = _corn_render_corn_hle


_original_render_info = ChatGPTN64EmuGUI.render_info

def _corn_render_info(self: "ChatGPTN64EmuGUI", s: Dict[str, object]) -> None:
...
ChatGPTN64EmuGUI.render_info = _corn_render_info


_original_gui_load_rom = ChatGPTN64EmuGUI.load_rom
...
def _sm64_signature_score(core: "ChatGPTN64EmuCore") -> Tuple[int, str]:
...
def _sm64_ensure_attrs(core: "ChatGPTN64EmuCore") -> None:
...
def _sm64_update_detection(core: "ChatGPTN64EmuCore") -> Tuple[int, str]:
...
def _sm64_is_family(core: "ChatGPTN64EmuCore") -> bool:
    _sm64_ensure_attrs(core)
...
def _sm64_install_universal_boottable(core: "ChatGPTN64EmuCore") -> None:
    _sm64_ensure_attrs(core)
...
_previous_sm64_configure_hle_profile = ChatGPTN64EmuCore.configure_hle_profile

def _sm64_configure_hle_profile(self: "ChatGPTN64EmuCore") -> None:
...
ChatGPTN64EmuCore.configure_hle_profile = _sm64_configure_hle_profile


_previous_sm64_load_rom_bytes = ChatGPTN64EmuCore.load_rom_bytes

def _sm64_load_rom_bytes(self: "ChatGPTN64EmuCore", raw: bytearray | bytes, name: str = "memory.rom", path: str = "") -> Dict[str, object]:
...
ChatGPTN64EmuCore.load_rom_bytes = _sm64_load_rom_bytes


_previous_sm64_reset = ChatGPTN64EmuCore.reset

def _sm64_reset(self: "ChatGPTN64EmuCore") -> None:
...
ChatGPTN64EmuCore.reset = _sm64_reset


_previous_sm64_boot = ChatGPTN64EmuCore.boot

def _sm64_boot(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
...
ChatGPTN64EmuCore.boot = _sm64_boot


def _sm64_tick_universal_hle(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    _sm64_ensure_attrs(self)
    self.frame_count += 1
    self.vi_count += 1
    self.hle_presented_frames += 1
...
_previous_sm64_tick_corn_hle = ChatGPTN64EmuCore.tick_corn_hle

def _sm64_tick_corn_hle(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    if self.hle_enabled and _sm64_is_family(self):
        return _sm64_tick_universal_hle(self)
    return _previous_sm64_tick_corn_hle(self)


ChatGPTN64EmuCore.tick_corn_hle = _sm64_tick_corn_hle


_previous_sm64_info = ChatGPTN64EmuCore.info

def _sm64_info(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    _sm64_ensure_attrs(self)
...
ChatGPTN64EmuCore.info = _sm64_info


_previous_sm64_render_info = ChatGPTN64EmuGUI.render_info

def _sm64_render_info(self: "ChatGPTN64EmuGUI", s: Dict[str, object]) -> None:
...
ChatGPTN64EmuGUI.render_info = _sm64_render_info
...
# ---------------------------------------------------------------------------
# CatHLE640.1 clean-room Corn public-feature layer
# ---------------------------------------------------------------------------
...
CATHLE640_VERSION = "cathle640.1"
CATHLE640_WINDOW_TITLE = "n64emuv0.1"
CATHLE640_ENGINE_FILE = "cathle640.1.py"
CATHLE640_PROFILE = "CATHLE640_1_CORN_PUBLIC_FEATURES_CLEANROOM_HLE"
CATHLE640_SOURCE_MODE = "CLEANROOM_PUBLIC_BEHAVIOR_COMPAT"
CATHLE640_FILES_OFF = True
CATHLE640_MONOLITHIC_NO_PLUGIN = True
CATHLE640_FAST_COMMERCIAL_BOOT = True
CATHLE640_DX61_FACADE = True
CATHLE640_SOUND_HLE = True
CATHLE640_INI_COMPAT = True
CATHLE640_STATIC_BLOCK_HLE = True
CATHLE640_GUI_TITLE = "n64emuv0.1"
CATHLE640_AUDIO_RING_LIMIT = 4096
CATHLE640_TARGET_PLATFORM = "python3.14 single-file"

APP_NAME = CATHLE640_WINDOW_TITLE
ENGINE_FILE = CATHLE640_ENGINE_FILE
CLEANROOM_PROFILE = CATHLE640_PROFILE

CATHLE640_PUBLIC_CORN_FEATURES = (
...
CATHLE640_FEATURE_FLAGS = {
    "files_off": CATHLE640_FILES_OFF,
    "no_plugin_system": CATHLE640_MONOLITHIC_NO_PLUGIN,
    "fast_commercial_hle": CATHLE640_FAST_COMMERCIAL_BOOT,
    "directx61_graphics_facade": CATHLE640_DX61_FACADE,
    "sound_hle": CATHLE640_SOUND_HLE,
    "corn_ini_embedded": CATHLE640_INI_COMPAT,
    "static_block_hle": CATHLE640_STATIC_BLOCK_HLE,
...
CATHLE640_CORN_PROFILE_UPDATES = {
    MARIO_KART_PROFILE: {
        "display": "MARIO KART 64",
        "short": "MK64",
        "prefix": "MK64",
...
for _cathle_profile_name, _cathle_profile in CATHLE640_CORN_PROFILE_UPDATES.items():
    if _cathle_profile_name in CORN_HLE_PROFILES:
        _merged = dict(CORN_HLE_PROFILES[_cathle_profile_name])
        for _key, _value in _cathle_profile.items():
            if _key in ("cart_ids", "title_keys"):
                _merged[_key] = tuple(dict.fromkeys(tuple(_merged.get(_key, ())) + tuple(_value)))
            else:
                _merged[_key] = _value
        CORN_HLE_PROFILES[_cathle_profile_name] = _merged
    else:
        CORN_HLE_PROFILES[_cathle_profile_name] = dict(_cathle_profile)
    CORN_HLE_PROFILES[_cathle_profile_name]["features"] = CATHLE640_PUBLIC_CORN_FEATURES
    CORN_HLE_PROFILES[_cathle_profile_name]["release_compat"] = tuple(CATHLE640_RELEASE_COMPAT.keys())
    CORN_HLE_PROFILES[_cathle_profile_name]["directx61_facade"] = True
    CORN_HLE_PROFILES[_cathle_profile_name]["sound_hle"] = True
    CORN_HLE_PROFILES[_cathle_profile_name]["no_plugin"] = True
...
def _cathle640_valid_n64_rom(core: "ChatGPTN64EmuCore") -> bool:
    return bool(getattr(core, "rom_size", 0) > 0 and bytes(getattr(core, "rom", bytearray())[:4]) == bytes.fromhex("80371240"))


def _cathle640_profile_for(core: "ChatGPTN64EmuCore") -> Optional[Dict[str, object]]:
    game = getattr(core, "hle_game", GENERIC_PROFILE)
    profile = CORN_HLE_PROFILES.get(game)
    if profile is not None and (game != GENERIC_CORN_COMMERCIAL_PROFILE or _cathle640_valid_n64_rom(core)):
        return profile
    if _cathle640_valid_n64_rom(core):
...
_corn_profile_for = _cathle640_profile_for


def _cathle640_ensure_attrs(core: "ChatGPTN64EmuCore") -> None:
    _corn_ensure_attrs(core)
    _sm64_ensure_attrs(core)
    if not hasattr(core, "cathle640_version"):
        core.cathle640_version = CATHLE640_VERSION
    if not hasattr(core, "cathle640_feature_flags"):
        core.cathle640_feature_flags = dict(CATHLE640_FEATURE_FLAGS)
    if not hasattr(core, "cathle640_public_features"):
        core.cathle640_public_features = tuple(CATHLE640_PUBLIC_CORN_FEATURES)
    if not hasattr(core, "cathle640_release_compat"):
        core.cathle640_release_compat = dict(CATHLE640_RELEASE_COMPAT)
    if not hasattr(core, "cathle640_directx61_frames"):
        core.cathle640_directx61_frames = 0
    if not hasattr(core, "cathle640_sound_frames"):
        core.cathle640_sound_frames = 0
    if not hasattr(core, "cathle640_static_blocks"):
        core.cathle640_static_blocks = 0
    if not hasattr(core, "cathle640_commercial_boots"):
        core.cathle640_commercial_boots = 0
    if not hasattr(core, "cathle640_ini_hits"):
        core.cathle640_ini_hits = 0
    if not hasattr(core, "cathle640_dx_backend"):
        core.cathle640_dx_backend = "DirectX 6.1 facade -> Tk Canvas preview"
    if not hasattr(core, "cathle640_audio_backend"):
        core.cathle640_audio_backend = "AI HLE ring buffer"
    if not hasattr(core, "cathle640_audio_ring"):
        core.cathle640_audio_ring = []
    if not hasattr(core, "cathle640_last_feature"):
        core.cathle640_last_feature = "ready"
    if not hasattr(core, "cathle640_source_mode"):
        core.cathle640_source_mode = CATHLE640_SOURCE_MODE
    core.corn_profile = CATHLE640_PROFILE
    core.corn_source_imported = False
    core.corn_no_plugin_system = True
    core.corn_no_plugin = True
    core.corn_import_mode = CATHLE640_SOURCE_MODE
    core.corn_hle_profile = "CORN_PUBLIC_FEATURES_CATHLE640_CLEANROOM"
    core.corn_video_backend = "DirectX6.1-facade/TkCanvas-no-plugin"
    core.compat_profile = CATHLE640_PROFILE
    core.engine_file = CATHLE640_ENGINE_FILE
...
def _cathle640_audio_push(core: "ChatGPTN64EmuCore", count: int = 735) -> None:
    _cathle640_ensure_attrs(core)
    seed = (int(getattr(core, "frame_count", 0)) * 17 + int(getattr(core, "audio_samples", 0))) & 0xFF
    samples = min(max(0, int(count)), 1470)
    ring = core.cathle640_audio_ring
    for i in range(samples // 64 + 1):
        ring.append((seed + i * 13) & 0xFF)
    if len(ring) > CATHLE640_AUDIO_RING_LIMIT:
        del ring[:len(ring) - CATHLE640_AUDIO_RING_LIMIT]
    core.cathle640_sound_frames += 1
    core.cathle640_last_feature = "sound-HLE"


def _cathle640_apply_profile_state(core: "ChatGPTN64EmuCore", profile: Dict[str, object]) -> None:
    _cathle640_ensure_attrs(core)
...
_cathle640_previous_is_corn_hle_game = ChatGPTN64EmuCore.is_corn_hle_game

def _cathle640_is_corn_hle_game(self: "ChatGPTN64EmuCore") -> bool:
    return bool(getattr(self, "hle_enabled", True) and _cathle640_profile_for(self) is not None)


ChatGPTN64EmuCore.is_corn_hle_game = _cathle640_is_corn_hle_game


_cathle640_previous_configure_hle_profile = ChatGPTN64EmuCore.configure_hle_profile

def _cathle640_configure_hle_profile(self: "ChatGPTN64EmuCore") -> None:
    _cathle640_previous_configure_hle_profile(self)
    _cathle640_ensure_attrs(self)
    ...
    for profile_name, profile in CORN_HLE_PROFILES.items():
        ...
        if any(key and key in combined for key in title_hits) or (cart_key and cart_key in cart_hits):
            ...
            break
    if selected is None and _cathle640_valid_n64_rom(self):
        selected = GENERIC_CORN_COMMERCIAL_PROFILE
    if selected is not None:
        profile = CORN_HLE_PROFILES[selected]
        self.hle_game = selected
        self.hle_game_confidence = "CATHLE640-CORN-INI" if selected != GENERIC_CORN_COMMERCIAL_PROFILE else "CATHLE640-GENERIC-CART"
        ...
        self.hle_boot_state = "CatHLE640 profile " + self.hle_title_short
        self.hle_boot_progress = 0
        self.hle_video_mode = self.hle_video_tag + "_READY"
        self.cathle640_ini_hits += 1


ChatGPTN64EmuCore.configure_hle_profile = _cathle640_configure_hle_profile


_cathle640_previous_load_rom_bytes = ChatGPTN64EmuCore.load_rom_bytes

def _cathle640_load_rom_bytes(self: "ChatGPTN64EmuCore", raw: bytearray | bytes, name: str = "memory.rom", path: str = "") -> Dict[str, object]:
    info = _cathle640_previous_load_rom_bytes(self, raw, name, path)
    _cathle640_ensure_attrs(self)
    profile = _cathle640_profile_for(self)
    if profile is not None and not _sm64_is_family(self):
        self.boot_status = "N64 HEADER OK | CATHLE640 CORN PUBLIC-FEATURE HLE READY"
        self.hle_video_mode = str(profile.get("prefix", "N64")) + "_READY"
        return self.info()
    return self.info() if profile is not None else info


ChatGPTN64EmuCore.load_rom_bytes = _cathle640_load_rom_bytes


_cathle640_previous_reset = ChatGPTN64EmuCore.reset

def _cathle640_reset(self: "ChatGPTN64EmuCore") -> None:
    _cathle640_previous_reset(self)
    _cathle640_ensure_attrs(self)
    self.cathle640_directx61_frames = 0
    self.cathle640_sound_frames = 0
    self.cathle640_static_blocks = 0
    self.cathle640_audio_ring.clear()
    self.cathle640_last_feature = "reset"


ChatGPTN64EmuCore.reset = _cathle640_reset


_cathle640_previous_boot = ChatGPTN64EmuCore.boot

def _cathle640_boot(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    info = _cathle640_previous_boot(self)
    _cathle640_ensure_attrs(self)
    profile = _cathle640_profile_for(self)
    if not info.get("booted") or profile is None or not self.hle_enabled:
        return self.info()
    _cathle640_apply_profile_state(self, profile)
    self.install_safe_hle_trampoline()
    self.install_boot_environment()
    _cathle640_apply_profile_state(self, profile)
    self.cpu.reset(self.boot_pc or int(profile.get("base_pc", 0x80000400)))
    ...
    self.booted = True
    self.running = True
    self.cathle640_commercial_boots += 1
    self.cathle640_static_blocks = max(1, self.cathle640_static_blocks)
    self.cathle640_last_feature = "fast-commercial-boot"
    display = str(profile.get("display", self.hle_game))
    if _sm64_is_family(self):
        if "CatHLE640" not in self.boot_status:
            self.boot_status = str(self.boot_status) + " | CatHLE640 Corn public features"
    else:
        self.hle_boot_state = "CIC/PIF handshake"
        self.hle_boot_progress = 8
        self.hle_video_mode = str(profile.get("prefix", "N64")) + "_IPL3"
        self.cpu.last_decode = "CatHLE640 Corn public-feature static boot"
        self.boot_status = f"{display} CATHLE640/CORN-HLE BOOTED AT PC 0x{self.boot_pc:08X}"
    self.hle_note("CATHLE640_FAST_COMMERCIAL_BOOT")
    return self.info()


ChatGPTN64EmuCore.boot = _cathle640_boot


_cathle640_previous_tick_corn_hle = ChatGPTN64EmuCore.tick_corn_hle

def _cathle640_tick_corn_hle(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    info = _cathle640_previous_tick_corn_hle(self)
    _cathle640_ensure_attrs(self)
    profile = _cathle640_profile_for(self)
    if not self.booted or profile is None:
        return info
    _cathle640_apply_profile_state(self, profile)
    self.cathle640_directx61_frames += 1
    self.cathle640_static_blocks += 3 if self.ultra_speed else 1
    _cathle640_audio_push(self, int(profile.get("audio", 735)))
    if self.frame_count % 4 == 0:
        self.hle_note("CATHLE640_DX61_TRIANGLE_FACADE")
    if self.frame_count % 5 == 0:
        self.hle_note("CATHLE640_SOUND_HLE_MIX")
    if self.frame_count % 9 == 0:
        self.hle_note("CATHLE640_NO_PLUGIN_SYNC")
    self.cathle640_last_feature = "dx61+sound+static-block"
    if not _sm64_is_family(self):
        display = str(profile.get("display", self.hle_game))
        self.boot_status = f"{display} CATHLE640/CORN-HLE: {self.hle_boot_state}"
    return self.info()


ChatGPTN64EmuCore.tick_corn_hle = _cathle640_tick_corn_hle


_cathle640_previous_info = ChatGPTN64EmuCore.info

def _cathle640_info(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    _cathle640_ensure_attrs(self)
    data = _cathle640_previous_info(self)
    profile = _cathle640_profile_for(self)
    data.update({
        "app_name": CATHLE640_WINDOW_TITLE,
        "engine_file": CATHLE640_ENGINE_FILE,
        "cathle640": True,
        "cathle640_version": CATHLE640_VERSION,
        "cathle640_profile": CATHLE640_PROFILE,
        "cathle640_source_mode": self.cathle640_source_mode,
        "cathle640_files_off": CATHLE640_FILES_OFF,
        "cathle640_window_title": CATHLE640_WINDOW_TITLE,
        "cathle640_public_features": ", ".join(CATHLE640_PUBLIC_CORN_FEATURES),
        "cathle640_release_compat": ", ".join(CATHLE640_RELEASE_COMPAT.keys()),
        "cathle640_feature_flags": dict(CATHLE640_FEATURE_FLAGS),
        "cathle640_directx61_facade": CATHLE640_DX61_FACADE,
        "cathle640_dx_backend": self.cathle640_dx_backend,
        "cathle640_sound_hle": CATHLE640_SOUND_HLE,
        "cathle640_audio_backend": self.cathle640_audio_backend,
        "cathle640_audio_ring_depth": len(self.cathle640_audio_ring),
        "cathle640_directx61_frames": self.cathle640_directx61_frames,
        "cathle640_sound_frames": self.cathle640_sound_frames,
        "cathle640_static_blocks": self.cathle640_static_blocks,
        "cathle640_commercial_boots": self.cathle640_commercial_boots,
        "cathle640_ini_hits": self.cathle640_ini_hits,
        "cathle640_last_feature": self.cathle640_last_feature,
        "cathle640_profile_display": str(profile.get("display")) if profile else "NONE",
        "corn_feature_complete_cleanroom": True,
        "corn_public_feature_count": len(CATHLE640_PUBLIC_CORN_FEATURES),
        "corn_no_plugin_system": True,
        "corn_source_imported": False,
...
ChatGPTN64EmuCore.info = _cathle640_info


_cathle640_previous_wrapper_corn_profile = N64CythonWrapper.corn_profile

def _cathle640_wrapper_corn_profile(self: "N64CythonWrapper") -> Dict[str, object]:
    info = self.core.info()
    return {
        "app": CATHLE640_WINDOW_TITLE,
        "version": CATHLE640_VERSION,
        "profile": info.get("cathle640_profile"),
        "source_mode": info.get("cathle640_source_mode"),
        "mode": info.get("corn_import_mode"),
        "no_plugin": True,
        "directx61_facade": info.get("cathle640_directx61_facade"),
        "sound_hle": info.get("cathle640_sound_hle"),
        "output": (info.get("hle_output_width"), info.get("hle_output_height")),
        "game": info.get("hle_game"),
        "features": info.get("cathle640_public_features"),
    }


N64CythonWrapper.corn_profile = _cathle640_wrapper_corn_profile


_cathle640_previous_render_corn_hle = ChatGPTN64EmuGUI.render_corn_hle

def _cathle640_render_corn_hle(self: "ChatGPTN64EmuGUI", s: Dict[str, object]) -> None:
    _cathle640_previous_render_corn_hle(self, s)
    try:
        w, h = self.screen_width, self.screen_height
        self.screen.create_text(
            w // 2,
            26,
            text="cathle640.1 / Corn public features / files off",
            fill=WHITE,
        ...
        self.screen.create_text(
            ...
            text="DX6.1 facade {} | sound {} | no plugins".format(
                s.get("cathle640_directx61_frames", 0),
                s.get("cathle640_sound_frames", 0),
            )[:58],
            fill=YELLOW,
            font=("Consolas", 7, "normal"),
        )
...
ChatGPTN64EmuGUI.render_corn_hle = _cathle640_render_corn_hle


_cathle640_previous_render_info = ChatGPTN64EmuGUI.render_info

def _cathle640_render_info(self: "ChatGPTN64EmuGUI", s: Dict[str, object]) -> None:
    if s.get("cathle640"):
        size = int(s.get("rom_size", 0) or 0)
        size_mb = size / (1024 * 1024) if size else 0
        ...
        text = (
            ...
            f"PC {int(s.get('pc',0)):08X} NEXT {int(s.get('next_pc',0)):08X} | 4K {s.get('hle4k_target_width')}x{s.get('hle4k_target_height')}\n"
            f"Corn pub: no-plugin DX6.1 facade sound-HLE | INI {s.get('cathle640_ini_hits')}\n"
            f"OP {int(s.get('last_opcode',0)):08X} | {decode}\n"
            f"blocks {s.get('cathle640_static_blocks')} dx {s.get('cathle640_directx61_frames')} aud {s.get('cathle640_sound_frames')} FPS 60"
        )
        self.info_label.config(text=text)
        return
    _cathle640_previous_render_info(self, s)


ChatGPTN64EmuGUI.render_info = _cathle640_render_info


_cathle640_previous_build_ui = ChatGPTN64EmuGUI.build_ui

def _cathle640_build_ui(self: "ChatGPTN64EmuGUI") -> None:
    _cathle640_previous_build_ui(self)
    try:
        self.root.title(CATHLE640_WINDOW_TITLE)
    except Exception:
        pass


ChatGPTN64EmuGUI.build_ui = _cathle640_build_ui


class CatHLE640RomBooter:
    """Headless ROM booter exposing the CatHLE640.1 Corn public-feature path."""
...
Cathle640RomBooter = CatHLE640RomBooter


def boot_cathle640_file(path: str, frames: int = 60) -> Dict[str, object]:
    return CatHLE640RomBooter().boot_path(path, frames)


def boot_cathle640_bytes(data: bytearray | bytes, name: str = "memory.z64", frames: int = 60) -> Dict[str, object]:
    return CatHLE640RomBooter().boot_bytes(data, name, frames)


def _make_synthetic_cathle640_rom(title: bytes = b"WAVE RACE 64", cart_id: bytes = b"WR", endian: str = "z64") -> bytearray:
    rom = bytearray(0x200000)
    rom[0:4] = bytes.fromhex("80371240")
    put_be32(rom, 0x04, 0x0000000F)
    put_be32(rom, 0x08, 0x80000400)
    ...
    for i, word in enumerate((0x3C088040, 0x8D090000, 0x25290001, 0xAD090000, 0x08000101, 0x00000000)):
        put_be32(rom, 0x400 + i * 4, word)
    marker = b"CATHLE640 CORN PUBLIC FEATURE CLEANROOM BOOT MARKER"
    rom[0x1000:0x1000 + len(marker)] = marker
    out = bytearray(rom)
    if endian.lower() == "v64":
        for i in range(0, len(out) - 1, 2):
...
_cathle640_previous_selftest = _selftest

def _selftest() -> None:
    _cathle640_previous_selftest()

    core = ChatGPTN64EmuCore()
    info = core.load_rom_bytes(_make_synthetic_cathle640_rom(b"WAVE RACE 64", b"WR", "z64"), "WaveRace64.z64")
    assert info["rom_type"] == "Z64 BIG-ENDIAN", info
    assert info["hle_game"] == WAVE_RACE_PROFILE, info
    assert info["cathle640"] is True, info
    assert info["cathle640_directx61_facade"] is True, info
    info = core.boot()
    assert info["booted"] is True, info
    assert "CATHLE640" in str(info["boot"]), info
    for _ in range(16):
        info = core.tick_frame()
    assert int(info["frame"]) == 16, info
    assert int(info["cathle640_directx61_frames"]) >= 16, info
    assert int(info["cathle640_sound_frames"]) >= 16, info
    assert int(info["cathle640_static_blocks"]) > 0, info
    assert info["corn_no_plugin_system"] is True, info

    core = ChatGPTN64EmuCore()
    info = core.load_rom_bytes(_make_synthetic_cathle640_rom(b"MYSTERY CART", b"ZZ", "n64"), "MysteryCart.n64")
    assert info["rom_type"] == "N64 LITTLE-ENDIAN", info
    ...
    for _ in range(4):
        info = core.tick_frame()
    assert int(info["cathle640_directx61_frames"]) >= 4, info

    wrapper = N64CythonWrapper()
    info = wrapper.load_rom_bytes(bytes(_make_synthetic_cathle640_rom(b"STAR FOX 64", b"SF", "v64")), "StarFox64.v64")
    assert info["rom_type"] == "V64 BYTE-SWAPPED", info
...
def _print_boot_summary(info: Dict[str, object]) -> None:
    print("cathle640.1 boot summary")
...
CATHLE011B_VERSION = "cathle0.1.1b"
CATHLE011B_ENGINE_FILE = "cathle0.1.1b.py"
CATHLE011B_WINDOW_TITLE = "n64emuv0.1"
CATHLE011B_PROFILE = "CATHLE64_0_1_1B_CORN_PUBLIC_FEATURES_CLEANROOM_RECOMPILER_HLE"
CATHLE011B_SOURCE_MODE = "CLEANROOM_PUBLIC_BEHAVIOR_COMPAT_NO_CORN_SOURCE"
CATHLE011B_RECOMPILER_MODE = "IN_MEMORY_STATIC_BLOCK_HLE_RECOMPILER_FILES_OFF"
CATHLE011B_COMMERCIAL_MODE = "COMMERCIAL_CART_FAST_BOOT_HLE"
CATHLE011B_SCAN_LIMIT = 2 * 1024 * 1024
CATHLE011B_MAX_COMPILED_BLOCKS = 8192
CATHLE011B_BLOCK_BYTES = 0x20
CATHLE011B_BLOCK_OPS = 8
CATHLE011B_AUDIO_RATE = 44100
CATHLE011B_DIRECTX_TARGET = "DirectX 6.1 public-feature facade via TkCanvas"
CATHLE011B_FILES_OFF = True
CATHLE011B_ROM_EXPORTS = False
CATHLE011B_CLEANROOM = True

APP_NAME = CATHLE011B_WINDOW_TITLE
ENGINE_FILE = CATHLE011B_ENGINE_FILE
CLEANROOM_PROFILE = CATHLE011B_PROFILE

CATHLE011B_PUBLIC_CORN_FEATURES = tuple(dict.fromkeys(tuple(CATHLE640_PUBLIC_CORN_FEATURES) + (
...
CATHLE011B_RELEASE_COMPAT = dict(CATHLE640_RELEASE_COMPAT)
CATHLE011B_RELEASE_COMPAT.update({
    "cathle0.1.1b": "clean-room Python 3.14 files-off Corn public-feature HLE/recompiler layer",
...
CATHLE011B_FEATURE_FLAGS = dict(CATHLE640_FEATURE_FLAGS)
CATHLE011B_FEATURE_FLAGS.update({
    "version": CATHLE011B_VERSION,
    "engine_file": CATHLE011B_ENGINE_FILE,
    "cleanroom": CATHLE011B_CLEANROOM,
    "corn_source_imported": False,
    "rom_exports": CATHLE011B_ROM_EXPORTS,
    "recompiler_mode": CATHLE011B_RECOMPILER_MODE,
    "commercial_mode": CATHLE011B_COMMERCIAL_MODE,
    "audio_rate": CATHLE011B_AUDIO_RATE,
})
...
for _c011b_profile_name, _c011b_profile in CATHLE011B_EXTRA_PROFILES.items():
    ...
    _merged_profile["features"] = CATHLE011B_PUBLIC_CORN_FEATURES
    _merged_profile["release_compat"] = tuple(CATHLE011B_RELEASE_COMPAT.keys())
    _merged_profile["directx61_facade"] = True
    _merged_profile["sound_hle"] = True
    _merged_profile["no_plugin"] = True
    _merged_profile["recompiler"] = CATHLE011B_RECOMPILER_MODE
    _merged_profile["commercial_fastboot"] = True
    CORN_HLE_PROFILES[_c011b_profile_name] = _merged_profile

CORN_HLE_TITLES.update({name: str(profile.get("display", name)) for name, profile in CATHLE011B_EXTRA_PROFILES.items()})
CORN_HLE_TAGS.update({name: str(profile.get("prefix", "N64")) for name, profile in CATHLE011B_EXTRA_PROFILES.items()})
try:
    CORN_HLE_FAST_PROFILES = tuple(dict.fromkeys(tuple(CORN_HLE_FAST_PROFILES) + tuple(CORN_HLE_PROFILES.keys())))
except Exception:
    CORN_HLE_FAST_PROFILES = tuple(CORN_HLE_PROFILES.keys())
...
def _cathle011b_decode_word_name(word: int) -> str:
    o = ChatGPTN64EmuOpcode(word)
    if o.op == 0x00:
...
def _cathle011b_rom_hash(rom: bytearray | bytes) -> str:
    h = 0x811C9DC5
...
def _cathle011b_safe_identifier(value: object) -> str:
...
class Cathle011BCommercialRecompiler:
    ...
    def __init__(self, max_blocks: int = CATHLE011B_MAX_COMPILED_BLOCKS, scan_limit: int = CATHLE011B_SCAN_LIMIT):
    ...
    def compile(self, rom: bytearray | bytes, boot_pc: int = 0x80000400) -> List[Cathle011BCompiledBlock]:
        ...
        while off < limit and len(blocks) < self.max_blocks:
            ...
            has_branch = False
            nonzero = 0
            for i in range(CATHLE011B_BLOCK_OPS):
                at = off + i * 4
                if at + 3 >= len(rom):
                    break
                word = be32(rom, at)
                if word:
                    nonzero += 1
                name = _cathle011b_decode_word_name(word)
                names.append(name)
                h ^= word
                h = (h * 0x01000193) & MASK_32
...
def _cathle011b_profile_for(core: "ChatGPTN64EmuCore") -> Optional[Dict[str, object]]:
    profile = _cathle640_profile_for(core)
...
def _cathle011b_valid_rom(core: "ChatGPTN64EmuCore") -> bool:
    try:
        return bool(_cathle640_valid_n64_rom(core))
    except Exception:
        return bool(getattr(core, "rom", b"")[:4] == bytes.fromhex("80371240"))


def _cathle011b_ensure_attrs(core: "ChatGPTN64EmuCore") -> None:
    _cathle640_ensure_attrs(core)
    if not hasattr(core, "cathle011b_version"):
        core.cathle011b_version = CATHLE011B_VERSION
    if not hasattr(core, "cathle011b_recompiler"):
        core.cathle011b_recompiler = Cathle011BCommercialRecompiler()
    if not hasattr(core, "cathle011b_compiled_blocks"):
        core.cathle011b_compiled_blocks: List[Cathle011BCompiledBlock] = []
    if not hasattr(core, "cathle011b_recompiler_ready"):
        core.cathle011b_recompiler_ready = False
    if not hasattr(core, "cathle011b_recompile_hash"):
        core.cathle011b_recompile_hash = "00000000"
    if not hasattr(core, "cathle011b_recompile_title"):
        core.cathle011b_recompile_title = ""
    if not hasattr(core, "cathle011b_recompile_frames"):
        core.cathle011b_recompile_frames = 0
    if not hasattr(core, "cathle011b_recompile_ticks"):
        core.cathle011b_recompile_ticks = 0
    if not hasattr(core, "cathle011b_recompiler_mode"):
        core.cathle011b_recompiler_mode = CATHLE011B_RECOMPILER_MODE
    if not hasattr(core, "cathle011b_commercial_level"):
        core.cathle011b_commercial_level = "pending"
    if not hasattr(core, "cathle011b_compiled_ops"):
        core.cathle011b_compiled_ops = 0
    if not hasattr(core, "cathle011b_recompile_notes"):
        core.cathle011b_recompile_notes: List[str] = []
    core.cathle640_version = CATHLE011B_VERSION
    core.cathle640_source_mode = CATHLE011B_SOURCE_MODE
    core.cathle640_feature_flags = dict(CATHLE011B_FEATURE_FLAGS)
    core.cathle640_public_features = tuple(CATHLE011B_PUBLIC_CORN_FEATURES)
    core.cathle640_release_compat = dict(CATHLE011B_RELEASE_COMPAT)
    core.corn_profile = CATHLE011B_PROFILE
    core.corn_import_mode = CATHLE011B_SOURCE_MODE
    core.corn_hle_profile = "CORN_PUBLIC_FEATURES_CATHLE011B_CLEANROOM_RECOMPILER"
    core.compat_profile = CATHLE011B_PROFILE
    core.engine_file = CATHLE011B_ENGINE_FILE
    core.corn_source_imported = False
    core.corn_no_plugin_system = True


def _cathle011b_select_profile(core: "ChatGPTN64EmuCore") -> Optional[str]:
    ...
    for profile_name, profile in CORN_HLE_PROFILES.items():
        ...
        if any(key and key in combined for key in title_hits) or (cart_key and cart_key in cart_hits):
            return profile_name
    if _cathle011b_valid_rom(core):
        return GENERIC_CORN_COMMERCIAL_PROFILE
    return None


def _cathle011b_compile_loaded_rom(core: "ChatGPTN64EmuCore", force: bool = False) -> List[Cathle011BCompiledBlock]:
    _cathle011b_ensure_attrs(core)
    if not _cathle011b_valid_rom(core):
        core.cathle011b_recompiler_ready = False
        core.cathle011b_compiled_blocks = []
        core.cathle011b_compiled_ops = 0
        core.cathle011b_commercial_level = "not-n64"
        return []
    rom_hash = _cathle011b_rom_hash(core.rom)
    if (not force) and core.cathle011b_recompiler_ready and core.cathle011b_recompile_hash == rom_hash:
        return core.cathle011b_compiled_blocks
    blocks = core.cathle011b_recompiler.compile(core.rom, core.header.boot_address or 0x80000400)
    core.cathle011b_compiled_blocks = blocks
    core.cathle011b_recompile_hash = rom_hash
    core.cathle011b_recompile_title = getattr(core.header, "title", "N64") or "N64"
    core.cathle011b_compiled_ops = sum(block.words for block in blocks)
    core.cathle011b_recompiler_ready = True
    title_slug = _cathle011b_safe_identifier(core.cathle011b_recompile_title)
    core.cathle011b_commercial_level = "commercial-hle:%s:%s-blocks" % (title_slug, len(blocks))
    core.cathle011b_recompile_notes = [
        "files_off",
        "no_rom_export",
        "in_memory_blocks=%d" % len(blocks),
        "hash=%s" % rom_hash,
    ]
    core.hle_note("CATHLE011B_ROM_RECOMPILED_IN_MEMORY")
    return blocks


def _cathle011b_apply_runtime(core: "ChatGPTN64EmuCore", profile: Optional[Dict[str, object]]) -> None:
    _cathle011b_ensure_attrs(core)
    if profile is None:
        profile = CORN_HLE_PROFILES.get(GENERIC_CORN_COMMERCIAL_PROFILE, {})
    _cathle640_apply_profile_state(core, profile)
    core.hle_output_width = HLE4K_TARGET_WIDTH
    core.hle_output_height = HLE4K_TARGET_HEIGHT
    core.hle4k_target_width = HLE4K_TARGET_WIDTH
    core.hle4k_target_height = HLE4K_TARGET_HEIGHT
...
_cathle011b_previous_configure_hle_profile = ChatGPTN64EmuCore.configure_hle_profile

def _cathle011b_configure_hle_profile(self: "ChatGPTN64EmuCore") -> None:
    _cathle011b_previous_configure_hle_profile(self)
    _cathle011b_ensure_attrs(self)
    selected = _cathle011b_select_profile(self)
    if selected is not None:
        profile = CORN_HLE_PROFILES[selected]
        self.hle_game = selected
        self.hle_game_confidence = "CATHLE011B-CORN-INI" if selected != GENERIC_CORN_COMMERCIAL_PROFILE else "CATHLE011B-GENERIC-COMMERCIAL-CART"
        self.hle_title_short = str(profile.get("display", self.hle_title_short))
        self.hle_video_tag = str(profile.get("prefix", self.hle_video_tag or "N64"))
...
ChatGPTN64EmuCore.configure_hle_profile = _cathle011b_configure_hle_profile


_cathle011b_previous_load_rom_bytes = ChatGPTN64EmuCore.load_rom_bytes

def _cathle011b_load_rom_bytes(self: "ChatGPTN64EmuCore", raw: bytearray | bytes, name: str = "memory.rom", path: str = "") -> Dict[str, object]:
    info = _cathle011b_previous_load_rom_bytes(self, raw, name, path)
    _cathle011b_ensure_attrs(self)
    profile = _cathle011b_profile_for(self)
    if _cathle011b_valid_rom(self):
        _cathle011b_compile_loaded_rom(self, force=True)
        tag = str((profile or {}).get("prefix", self.hle_video_tag or "N64"))
        self.boot_status = "N64 HEADER OK | CATHLE0.1.1B CORN PUBLIC-FEATURE RECOMPILER READY"
        self.hle_video_mode = tag + "_RECOMP_READY"
        return self.info()
    return info


ChatGPTN64EmuCore.load_rom_bytes = _cathle011b_load_rom_bytes


_cathle011b_previous_reset = ChatGPTN64EmuCore.reset

def _cathle011b_reset(self: "ChatGPTN64EmuCore") -> None:
    _cathle011b_previous_reset(self)
    _cathle011b_ensure_attrs(self)
    self.cathle011b_compiled_blocks = []
    self.cathle011b_recompiler_ready = False
    self.cathle011b_recompile_hash = "00000000"
    self.cathle011b_recompile_frames = 0
    self.cathle011b_recompile_ticks = 0
    self.cathle011b_compiled_ops = 0
    self.cathle011b_commercial_level = "reset"


ChatGPTN64EmuCore.reset = _cathle011b_reset


_cathle011b_previous_boot = ChatGPTN64EmuCore.boot

def _cathle011b_boot(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    info = _cathle011b_previous_boot(self)
    _cathle011b_ensure_attrs(self)
    if not _cathle011b_valid_rom(self):
        return self.info()
    profile = _cathle011b_profile_for(self) or CORN_HLE_PROFILES.get(GENERIC_CORN_COMMERCIAL_PROFILE)
    blocks = _cathle011b_compile_loaded_rom(self, force=False)
    _cathle011b_apply_runtime(self, profile)
    self.install_safe_hle_trampoline()
    self.install_boot_environment()
    ...
    self.running = True
    self.cathle640_commercial_boots += 1
    self.cathle640_static_blocks = max(self.cathle640_static_blocks, len(blocks))
    self.cathle011b_recompile_ticks += 1
    ...
    self.boot_status = f"{display} {compat_marker}CATHLE640/CATHLE0.1.1B COMMERCIAL-HLE BOOTED | {len(blocks)} blocks | files off"
    self.hle_note("CATHLE011B_COMMERCIAL_FAST_BOOT")
    return self.info()


ChatGPTN64EmuCore.boot = _cathle011b_boot


_cathle011b_previous_tick_corn_hle = ChatGPTN64EmuCore.tick_corn_hle

def _cathle011b_tick_corn_hle(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    info = _cathle011b_previous_tick_corn_hle(self)
    _cathle011b_ensure_attrs(self)
    if not self.booted or not _cathle011b_valid_rom(self):
        return info
    profile = _cathle011b_profile_for(self) or CORN_HLE_PROFILES.get(GENERIC_CORN_COMMERCIAL_PROFILE)
    blocks = _cathle011b_compile_loaded_rom(self, force=False)
    _cathle011b_apply_runtime(self, profile)
    ...
    self.cathle011b_recompile_frames += 1
    self.cathle011b_recompile_ticks += max(1, active.words)
    self.cathle640_static_blocks += max(1, active.words // 2)
    self.cathle640_directx61_frames += 1
    _cathle640_audio_push(self, int((profile or {}).get("audio", 735)))
    prefix = str((profile or {}).get("prefix", self.hle_video_tag or "N64"))
    ...
    self.hle_video_mode = prefix + "_" + suffix
    if self.frame_count % 2 == 0:
        self.hle_note("CATHLE011B_DX61_GRAPHICS_FACADE")
    if self.frame_count % 3 == 0:
        self.hle_note("CATHLE011B_SOUND_HLE_SELECTED_GAME")
    if self.frame_count % 4 == 0:
        self.hle_note("CATHLE011B_NO_PLUGIN_MONOLITHIC_SYNC")
    if self.frame_count % 8 == 0:
        self.process_pif_ram(force=True)
    ...
    display = str((profile or {}).get("display", self.hle_game or "N64 CART"))
    self.boot_status = "%s CATHLE0.1.1B: %s | block %04X" % (display, self.hle_boot_state, active.rom_offset)
    return self.info()


ChatGPTN64EmuCore.tick_corn_hle = _cathle011b_tick_corn_hle


_cathle011b_previous_info = ChatGPTN64EmuCore.info

def _cathle011b_info(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    _cathle011b_ensure_attrs(self)
    data = _cathle011b_previous_info(self)
    profile = _cathle011b_profile_for(self)
    blocks = getattr(self, "cathle011b_compiled_blocks", [])
    first_block = blocks[0].info() if blocks else {}
    data.update({
        "app_name": CATHLE011B_WINDOW_TITLE,
        "engine_file": CATHLE011B_ENGINE_FILE,
        "cathle011b": True,
        "cathle011b_version": CATHLE011B_VERSION,
        "cathle011b_profile": CATHLE011B_PROFILE,
        "cathle011b_source_mode": CATHLE011B_SOURCE_MODE,
        "cathle011b_recompiler_mode": CATHLE011B_RECOMPILER_MODE,
        "cathle011b_commercial_mode": CATHLE011B_COMMERCIAL_MODE,
        "cathle011b_features": ", ".join(CATHLE011B_PUBLIC_CORN_FEATURES),
        "cathle011b_feature_flags": dict(CATHLE011B_FEATURE_FLAGS),
        "cathle011b_release_compat": ", ".join(CATHLE011B_RELEASE_COMPAT.keys()),
        "cathle011b_recompiler_ready": bool(getattr(self, "cathle011b_recompiler_ready", False)),
        "cathle011b_compiled_blocks": len(blocks),
        "cathle011b_compiled_ops": int(getattr(self, "cathle011b_compiled_ops", 0)),
        "cathle011b_recompile_hash": getattr(self, "cathle011b_recompile_hash", "00000000"),
        "cathle011b_recompile_title": getattr(self, "cathle011b_recompile_title", ""),
        "cathle011b_recompile_frames": int(getattr(self, "cathle011b_recompile_frames", 0)),
        "cathle011b_recompile_ticks": int(getattr(self, "cathle011b_recompile_ticks", 0)),
        "cathle011b_commercial_level": getattr(self, "cathle011b_commercial_level", "pending"),
        "cathle011b_first_block": first_block,
        "cathle011b_no_plugin_system": True,
        "cathle011b_directx61_facade": True,
        "cathle011b_sound_hle": True,
        "cathle011b_files_off": True,
        "cathle011b_rom_exports": False,
        "cathle011b_profile_display": str(profile.get("display")) if profile else "GENERIC N64 COMMERCIAL CART",
        "corn_feature_complete_cleanroom": True,
        "corn_public_feature_count": len(CATHLE011B_PUBLIC_CORN_FEATURES),
        "corn_source_imported": False,
...
ChatGPTN64EmuCore.info = _cathle011b_info


_cathle011b_previous_wrapper_corn_profile = N64CythonWrapper.corn_profile

def _cathle011b_wrapper_corn_profile(self: "N64CythonWrapper") -> Dict[str, object]:
    info = self.core.info()
    return {
        "app": CATHLE011B_WINDOW_TITLE,
        "version": CATHLE011B_VERSION,
        "profile": info.get("cathle011b_profile"),
        "source_mode": info.get("cathle011b_source_mode"),
        "mode": info.get("corn_import_mode"),
        "no_plugin": True,
        "directx61_facade": True,
        "sound_hle": True,
        "recompiler": info.get("cathle011b_recompiler_mode"),
        "compiled_blocks": info.get("cathle011b_compiled_blocks"),
        "commercial_level": info.get("cathle011b_commercial_level"),
        "output": (info.get("hle_output_width"), info.get("hle_output_height")),
        "game": info.get("hle_game"),
        "features": info.get("cathle011b_features"),
    }


N64CythonWrapper.corn_profile = _cathle011b_wrapper_corn_profile


_cathle011b_previous_render_corn_hle = ChatGPTN64EmuGUI.render_corn_hle

def _cathle011b_render_corn_hle(self: "ChatGPTN64EmuGUI", s: Dict[str, object]) -> None:
    _cathle011b_previous_render_corn_hle(self, s)
    try:
        w, h = self.screen_width, self.screen_height
        self.screen.create_text(
            w // 2,
            38,
            text="cathle0.1.1b / Corn public features / in-memory recompiler",
            fill=WHITE,
            font=("Consolas", 7, "normal"),
        )
        self.screen.create_text(
            ...
            text="blocks {} ops {} hash {} | files off".format(
                s.get("cathle011b_compiled_blocks", 0),
                s.get("cathle011b_compiled_ops", 0),
                s.get("cathle011b_recompile_hash", "00000000"),
            )[:64],
            fill=YELLOW,
            font=("Consolas", 7, "normal"),
        )
...
ChatGPTN64EmuGUI.render_corn_hle = _cathle011b_render_corn_hle


_cathle011b_previous_render_info = ChatGPTN64EmuGUI.render_info

def _cathle011b_render_info(self: "ChatGPTN64EmuGUI", s: Dict[str, object]) -> None:
    if s.get("cathle011b"):
        size = int(s.get("rom_size", 0) or 0)
        size_mb = size / (1024 * 1024) if size else 0
        decode = str(s.get("last_decode") or "---")[:38]
        ...
        text = (
            ...
            f"ROM {str(s.get('rom'))[:22]} | {size_mb:.2f} MB | {str(s.get('rom_type'))[:16]}\n"
            f"TITLE {title} | {str(s.get('cathle011b_profile_display'))[:24]}\n"
            f"PC {int(s.get('pc',0)):08X} NEXT {int(s.get('next_pc',0)):08X} | 4K {s.get('hle4k_target_width')}x{s.get('hle4k_target_height')}\n"
            f"Corn pub: fast commercial boot | DX6.1 facade | sound | no plugins\n"
            f"recomp {s.get('cathle011b_compiled_blocks')} blocks {s.get('cathle011b_compiled_ops')} ops | hash {s.get('cathle011b_recompile_hash')}\n"
            f"OP {int(s.get('last_opcode',0)):08X} | {decode} | FPS 60"
        )
        self.info_label.config(text=text)
        return
    _cathle011b_previous_render_info(self, s)


ChatGPTN64EmuGUI.render_info = _cathle011b_render_info


_cathle011b_previous_build_ui = ChatGPTN64EmuGUI.build_ui

def _cathle011b_build_ui(self: "ChatGPTN64EmuGUI") -> None:
    _cathle011b_previous_build_ui(self)
    try:
        self.root.title(CATHLE011B_WINDOW_TITLE)
    except Exception:
        pass


ChatGPTN64EmuGUI.build_ui = _cathle011b_build_ui


class CatHLE011BRomBooter:
    """Headless ROM booter exposing the CatHLE64 0.1.1b files-off path."""
    ...
    def compile_path(self, path: str) -> Dict[str, object]:
        self.core.load_rom(path)
        _cathle011b_compile_loaded_rom(self.core, force=True)
        return self.core.info()
...
def boot_cathle011b_file(path: str, frames: int = 60) -> Dict[str, object]:
    return CatHLE011BRomBooter().boot_path(path, frames)


def boot_cathle011b_bytes(data: bytearray | bytes, name: str = "memory.z64", frames: int = 60) -> Dict[str, object]:
    return CatHLE011BRomBooter().boot_bytes(data, name, frames)


def compile_cathle011b_file(path: str) -> Dict[str, object]:
    return CatHLE011BRomBooter().compile_path(path)


# Keep older import helpers useful, but route them through the 0.1.1b booter.
boot_rom_file = boot_cathle011b_file
boot_rom_bytes = boot_cathle011b_bytes
boot_cathle640_file = boot_cathle011b_file
boot_cathle640_bytes = boot_cathle011b_bytes


def _make_synthetic_cathle011b_rom(title: bytes = b"PERFECT DARK", cart_id: bytes = b"PD", endian: str = "z64") -> bytearray:
    rom = _make_synthetic_cathle640_rom(title, cart_id, "z64")
    marker = b"CATHLE011B IN MEMORY COMMERCIAL RECOMPILER FILES OFF"
...
_cathle011b_previous_selftest = _selftest

def _selftest() -> None:
    _cathle011b_previous_selftest()

    core = ChatGPTN64EmuCore()
    info = core.load_rom_bytes(_make_synthetic_cathle011b_rom(b"PERFECT DARK", b"PD", "z64"), "PerfectDark.z64")
    assert info["engine_file"] == CATHLE011B_ENGINE_FILE, info
    assert info["cathle011b"] is True, info
    assert info["hle_game"] == PERFECT_DARK_PROFILE, info
    assert info["cathle011b_recompiler_ready"] is True, info
    assert int(info["cathle011b_compiled_blocks"]) > 0, info
    info = core.boot()
    assert info["booted"] is True, info
    assert "CATHLE0.1.1B" in str(info["boot"]), info
    for _ in range(10):
        info = core.tick_frame()
    assert int(info["cathle011b_recompile_frames"]) >= 10, info
    assert int(info["cathle011b_recompile_ticks"]) > 0, info
    assert info["cathle011b_no_plugin_system"] is True, info
    assert info["cathle011b_rom_exports"] is False, info

    core = ChatGPTN64EmuCore()
    info = core.load_rom_bytes(_make_synthetic_cathle011b_rom(b"MYSTERY CART", b"ZZ", "v64"), "MysteryCommercial.v64")
    assert info["rom_type"] == "V64 BYTE-SWAPPED", info
    assert info["hle_game"] == GENERIC_CORN_COMMERCIAL_PROFILE, info
    info = core.boot()
    assert info["booted"] is True, info
    ...
    wrapper = N64CythonWrapper()
    info = wrapper.load_rom_bytes(bytes(_make_synthetic_cathle011b_rom(b"PAPER MARIO", b"PM", "n64")), "PaperMario.n64")
    assert info["rom_type"] == "N64 LITTLE-ENDIAN", info
...
# ---------------------------------------------------------------------------
# CatHLE64 0.1.1c universal commercial loader/recompiler overlay
# ---------------------------------------------------------------------------
...
CATHLE011C_VERSION = "cathle0.1.1c"
CATHLE011C_PROFILE = "CATHLE64_0_1_1C_UNIVERSAL_COMMERCIAL_CLEANROOM_RECOMPILER_HLE"
CATHLE011C_SOURCE_MODE = "CLEANROOM_PUBLIC_BEHAVIOR_COMPAT_NO_CORN_SOURCE_UNIVERSAL"
CATHLE011C_RECOMPILER_MODE = "IN_MEMORY_MULTI_WINDOW_STATIC_BLOCK_RECOMPILER_FILES_OFF"
CATHLE011C_SCAN_LIMIT = 2 * 1024 * 1024
CATHLE011C_MAX_COMPILED_BLOCKS = 4096
CATHLE011C_ROM_EXTENSIONS = (".z64", ".v64", ".n64", ".rom", ".bin")
CATHLE011C_ARCHIVE_EXTENSIONS = (".zip",)
CATHLE011C_FILES_OFF = True
CATHLE011C_ROM_EXPORTS = False
...
for _c011c_profile_name, _c011c_profile in CATHLE011C_EXTRA_PROFILES.items():
    ...
    _merged011c["features"] = tuple(dict.fromkeys(tuple(_merged011c.get("features", ())) + tuple(CATHLE011B_PUBLIC_CORN_FEATURES) + (
        "universal-commercial-ROM-loader",
        "multi-window-in-memory-recompiler",
        "zip-container-ROM-input-no-extract",
    )))
    _merged011c["release_compat"] = tuple(dict.fromkeys(tuple(_merged011c.get("release_compat", ())) + tuple(CATHLE011B_RELEASE_COMPAT.keys()) + (CATHLE011C_VERSION,)))
    ...
    _merged011c["recompiler"] = CATHLE011C_RECOMPILER_MODE
    CORN_HLE_PROFILES[_c011c_profile_name] = _merged011c

CORN_HLE_TITLES.update({name: str(profile.get("display", name)) for name, profile in CATHLE011C_EXTRA_PROFILES.items()})
CORN_HLE_TAGS.update({name: str(profile.get("prefix", "N64")) for name, profile in CATHLE011C_EXTRA_PROFILES.items()})
try:
    CORN_HLE_FAST_PROFILES = tuple(dict.fromkeys(tuple(CORN_HLE_FAST_PROFILES) + tuple(CORN_HLE_PROFILES.keys())))
except Exception:
    CORN_HLE_FAST_PROFILES = tuple(CORN_HLE_PROFILES.keys())
...
def _cathle011c_profile_match(core: "ChatGPTN64EmuCore") -> Optional[str]:
    title_key = _corn_compact(getattr(core.header, "title", ""))
    name_key = _corn_compact(getattr(core, "rom_name", ""))
    cart_key = _corn_compact(getattr(core.header, "cart_id", ""))
    combined = title_key + name_key + cart_key
...
def _cathle011c_classify_block(names: Tuple[str, ...], words: Tuple[int, ...], has_branch: bool) -> str:
    if has_branch:
...
class Cathle011CUniversalCommercialRecompiler(Cathle011BCommercialRecompiler):
    ...
    def __init__(self, max_blocks: int = CATHLE011C_MAX_COMPILED_BLOCKS, scan_limit: int = CATHLE011C_SCAN_LIMIT):
    ...
    def _candidate_windows(self, rom: bytearray | bytes, boot_pc: int) -> Tuple[Tuple[int, int], ...]:
        ...
        if 0 <= boot_low < len(rom):
            starts.append(boot_low & ~(CATHLE011B_BLOCK_BYTES - 1))
        page = 0x400
        ...
        for start in starts:
            ...
            if start >= limit:
                continue
            start &= ~(CATHLE011B_BLOCK_BYTES - 1)
    ...
    def _compile_at(self, rom: bytearray | bytes, off: int, boot_pc: int, ordinal: int) -> Optional[Cathle011BCompiledBlock]:
        ...
        if nonzero <= 0 or meaningful <= 0:
            return None
        kind = _cathle011c_classify_block(tuple(names), tuple(words_seen), has_branch)
...
_cathle011c_previous_ensure_attrs = _cathle011b_ensure_attrs

def _cathle011b_ensure_attrs(core: "ChatGPTN64EmuCore") -> None:
    _cathle011c_previous_ensure_attrs(core)
    if not isinstance(getattr(core, "cathle011b_recompiler", None), Cathle011CUniversalCommercialRecompiler):
        # Upgrade old cores lazily. Keep already-compiled block lists intact until the next forced compile.
        core.cathle011b_recompiler = Cathle011CUniversalCommercialRecompiler()
        if not getattr(core, "cathle011b_recompiler_ready", False):
            core.cathle011b_compiled_blocks = []
    if not hasattr(core, "cathle011c_block_table"):
        core.cathle011c_block_table: Dict[int, Cathle011BCompiledBlock] = {}
    if not hasattr(core, "cathle011c_opcode_histogram"):
        core.cathle011c_opcode_histogram: Dict[str, int] = {}
    if not hasattr(core, "cathle011c_loader_status"):
        core.cathle011c_loader_status = "waiting"
    if not hasattr(core, "cathle011c_archive_input"):
        core.cathle011c_archive_input = ""
    if not hasattr(core, "cathle011c_block_cache_hits"):
        core.cathle011c_block_cache_hits = 0
    if not hasattr(core, "cathle011c_block_cache_misses"):
        core.cathle011c_block_cache_misses = 0
    core.cathle011c_version = CATHLE011C_VERSION
    core.cathle011c_profile = CATHLE011C_PROFILE
    core.cathle011c_source_mode = CATHLE011C_SOURCE_MODE
    core.cathle011c_recompiler_mode = CATHLE011C_RECOMPILER_MODE
    core.cathle011c_files_off = True
    core.cathle011c_rom_exports = False


_cathle011c_previous_select_profile = _cathle011b_select_profile

def _cathle011b_select_profile(core: "ChatGPTN64EmuCore") -> Optional[str]:
    selected = _cathle011c_previous_select_profile(core)
    matched = _cathle011c_profile_match(core)
...
_cathle011c_previous_compile_loaded_rom = _cathle011b_compile_loaded_rom

def _cathle011b_compile_loaded_rom(core: "ChatGPTN64EmuCore", force: bool = False) -> List[Cathle011BCompiledBlock]:
    _cathle011b_ensure_attrs(core)
    blocks = _cathle011c_previous_compile_loaded_rom(core, force=force)
    ...
    core.cathle011c_block_table = table
    core.cathle011c_opcode_histogram = histogram
    core.cathle011c_loader_status = "universal-commercial-loader-ready" if blocks else "no-blocks"
    core.cathle011b_recompiler_mode = CATHLE011C_RECOMPILER_MODE
    return blocks


_cathle011c_previous_load_rom = ChatGPTN64EmuCore.load_rom

def _cathle011c_extract_zip_rom(path: str) -> Tuple[bytearray, str]:
    import zipfile
    with zipfile.ZipFile(path, "r") as zf:
        ...
        for name in zf.namelist():
            ...
            if lower.endswith("/"):
                continue
            if lower.endswith(CATHLE011C_ROM_EXTENSIONS):
...
def _cathle011c_load_rom(self: "ChatGPTN64EmuCore", path: str) -> Dict[str, object]:
    if path and str(path).lower().endswith(CATHLE011C_ARCHIVE_EXTENSIONS):
        raw, name = _cathle011c_extract_zip_rom(path)
        info = self.load_rom_bytes(raw, name=name, path=path)
        _cathle011b_ensure_attrs(self)
        self.cathle011c_archive_input = name
        self.cathle011c_loader_status = "zip-rom-loaded-in-memory"
        return self.info()
    return _cathle011c_previous_load_rom(self, path)


ChatGPTN64EmuCore.load_rom = _cathle011c_load_rom


_cathle011c_previous_load_rom_bytes = ChatGPTN64EmuCore.load_rom_bytes

def _cathle011c_load_rom_bytes(self: "ChatGPTN64EmuCore", raw: bytearray | bytes, name: str = "memory.rom", path: str = "") -> Dict[str, object]:
    info = _cathle011c_previous_load_rom_bytes(self, raw, name, path)
    _cathle011b_ensure_attrs(self)
    if _cathle011b_valid_rom(self):
        selected = _cathle011b_select_profile(self)
        if selected is not None:
            ...
            self.hle_title_short = str(profile.get("display", self.hle_title_short))
            self.hle_video_tag = str(profile.get("prefix", self.hle_video_tag or "N64"))
        blocks = _cathle011b_compile_loaded_rom(self, force=True)
        self.cathle011c_loader_status = "valid-n64-universal-commercial-recompiler-ready"
        self.boot_status = "N64 HEADER OK | CATHLE0.1.1B/0.1.1C UNIVERSAL CLEANROOM RECOMPILER READY"
        self.hle_video_mode = str(self.hle_video_tag or "N64") + "_UNIVERSAL_RECOMP_READY"
        self.hle_note("CATHLE011C_UNIVERSAL_ROM_READY")
        return self.info()
    return info


ChatGPTN64EmuCore.load_rom_bytes = _cathle011c_load_rom_bytes


_cathle011c_previous_boot = ChatGPTN64EmuCore.boot

def _cathle011c_boot(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    info = _cathle011c_previous_boot(self)
    _cathle011b_ensure_attrs(self)
    if not _cathle011b_valid_rom(self):
        return info
    blocks = _cathle011b_compile_loaded_rom(self, force=False)
    ...
    self.cathle011c_loader_status = "universal-commercial-booted"
    compat_marker = "UNIVERSAL-HLE4K " if bool(globals().get("_sm64_is_family", lambda _c: False)(self)) else ""
    self.boot_status = f"{display} {compat_marker}CATHLE640/CATHLE0.1.1B/0.1.1C UNIVERSAL COMMERCIAL-HLE BOOTED | {len(blocks)} blocks | files off"
    self.hle_note("CATHLE011C_UNIVERSAL_COMMERCIAL_BOOT")
    return self.info()


ChatGPTN64EmuCore.boot = _cathle011c_boot


_cathle011c_previous_is_corn_hle_game = ChatGPTN64EmuCore.is_corn_hle_game

def _cathle011c_is_corn_hle_game(self: "ChatGPTN64EmuCore") -> bool:
    if bool(getattr(self, "hle_enabled", False)) and _cathle011b_valid_rom(self):
        return True
    return _cathle011c_previous_is_corn_hle_game(self)


ChatGPTN64EmuCore.is_corn_hle_game = _cathle011c_is_corn_hle_game


_cathle011c_previous_tick_corn_hle = ChatGPTN64EmuCore.tick_corn_hle

def _cathle011c_tick_corn_hle(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    info = _cathle011c_previous_tick_corn_hle(self)
    _cathle011b_ensure_attrs(self)
    if not self.booted or not _cathle011b_valid_rom(self):
        return info
    blocks = getattr(self, "cathle011b_compiled_blocks", [])
    if blocks:
        ...
        if active.pc in getattr(self, "cathle011c_block_table", {}):
            self.cathle011c_block_cache_hits += 1
        else:
            self.cathle011c_block_cache_misses += 1
        if self.frame_count % 5 == 0:
            self.hle_note("CATHLE011C_BLOCK_CACHE_HIT")
        if self.frame_count % 7 == 0:
            self.hle_note("CATHLE011C_UNIVERSAL_RSP_RDP_AUDIO_SYNC")
    self.cathle011c_loader_status = "universal-commercial-running"
    return self.info()


ChatGPTN64EmuCore.tick_corn_hle = _cathle011c_tick_corn_hle


_cathle011c_previous_info = ChatGPTN64EmuCore.info

def _cathle011c_info(self: "ChatGPTN64EmuCore") -> Dict[str, object]:
    _cathle011b_ensure_attrs(self)
    data = _cathle011c_previous_info(self)
    blocks = getattr(self, "cathle011b_compiled_blocks", [])
    histogram = getattr(self, "cathle011c_opcode_histogram", {})
    data.update({
        "cathle011c": True,
        "cathle011c_version": CATHLE011C_VERSION,
        "cathle011c_profile": CATHLE011C_PROFILE,
        "cathle011c_source_mode": CATHLE011C_SOURCE_MODE,
        "cathle011c_recompiler_mode": CATHLE011C_RECOMPILER_MODE,
        "cathle011c_universal_commercial_loader": True,
        "cathle011c_profile_catalog_size": len(CORN_HLE_PROFILES),
        "cathle011c_known_profile_count": len(CATHLE011C_EXTRA_PROFILES),
        "cathle011c_block_table_entries": len(getattr(self, "cathle011c_block_table", {})),
        "cathle011c_block_cache_hits": int(getattr(self, "cathle011c_block_cache_hits", 0)),
        "cathle011c_block_cache_misses": int(getattr(self, "cathle011c_block_cache_misses", 0)),
        "cathle011c_loader_status": getattr(self, "cathle011c_loader_status", "waiting"),
        "cathle011c_archive_input": getattr(self, "cathle011c_archive_input", ""),
        "cathle011c_top_block_kinds": ", ".join(
            f"{k}={v}" for k, v in sorted(
                ((k, v) for k, v in histogram.items() if not k.startswith("op:")),
                key=lambda item: (-item[1], item[0])
            )[:6]
        ),
        "cathle011c_files_off": CATHLE011C_FILES_OFF,
        "cathle011c_rom_exports": CATHLE011C_ROM_EXPORTS,
        "cathle011c_valid_n64_rom": _cathle011b_valid_rom(self),
        "cathle011c_loaded_block_bytes": sum(max(1, block.words) * 4 for block in blocks),
        "corn_source_imported": False,
...
ChatGPTN64EmuCore.info = _cathle011c_info


_cathle011c_previous_wrapper_corn_profile = N64CythonWrapper.corn_profile

def _cathle011c_wrapper_corn_profile(self: "N64CythonWrapper") -> Dict[str, object]:
    data = _cathle011c_previous_wrapper_corn_profile(self)
    info = self.core.info()
    data.update({
        "universal_loader": info.get("cathle011c_universal_commercial_loader"),
        "cathle011c_version": info.get("cathle011c_version"),
        "block_table_entries": info.get("cathle011c_block_table_entries"),
        "profile_catalog_size": info.get("cathle011c_profile_catalog_size"),
        "loader_status": info.get("cathle011c_loader_status"),
    })
    return data


N64CythonWrapper.corn_profile = _cathle011c_wrapper_corn_profile


def _cathle011c_gui_load_rom(self: "ChatGPTN64EmuGUI") -> None:
    path = filedialog.askopenfilename(
        title="Open N64 ROM",
    ...
    try:
        ...
        self.rom_list.insert(tk.END, f"CIC: {info['cic']}")
        self.rom_list.insert(tk.END, f"HLE: {info.get('hle_game')}")
        self.rom_list.insert(tk.END, f"blocks: {info.get('cathle011b_compiled_blocks')}")
...
try:
    ChatGPTN64EmuGUI.load_rom = _cathle011c_gui_load_rom
except Exception:
    pass


_cathle011c_previous_render_info = ChatGPTN64EmuGUI.render_info

def _cathle011c_render_info(self: "ChatGPTN64EmuGUI", s: Dict[str, object]) -> None:
    if s.get("cathle011c"):
        size = int(s.get("rom_size", 0) or 0)
        size_mb = size / (1024 * 1024) if size else 0
        ...
        text = (
            ...
            f"recomp {s.get('cathle011b_compiled_blocks')} blocks {s.get('cathle011b_compiled_ops')} ops | table {s.get('cathle011c_block_table_entries')}\n"
            f"OP {int(s.get('last_opcode',0)):08X} | {decode} | FPS 60"
        )
        self.info_label.config(text=text)
        return
    _cathle011c_previous_render_info(self, s)


ChatGPTN64EmuGUI.render_info = _cathle011c_render_info
...
Cathle011CUniversalRomBooter = CatHLE011BRomBooter
boot_cathle011c_file = boot_cathle011b_file
boot_cathle011c_bytes = boot_cathle011b_bytes
compile_cathle011c_file = compile_cathle011b_file


_cathle011c_previous_selftest = _selftest

def _selftest() -> None:
    _cathle011c_previous_selftest()

    core = ChatGPTN64EmuCore()
    info = core.load_rom_bytes(_make_synthetic_cathle011b_rom(b"BANJO TOOIE", b"BT", "z64"), "BanjoTooie.z64")
    assert info["cathle011c"] is True, info
    assert info["hle_game"] == BANJO_TOOIE_PROFILE, info
    assert info["cathle011c_universal_commercial_loader"] is True, info
    assert int(info["cathle011c_block_table_entries"]) > 0, info
    assert int(info["cathle011b_compiled_blocks"]) > 0, info
    info = core.boot()
    assert info["booted"] is True, info
    assert "CATHLE0.1.1B" in str(info["boot"]), info
    assert "0.1.1C" in str(info["boot"]) or info.get("cathle011c_version") == CATHLE011C_VERSION, info
    for _ in range(6):
        info = core.tick_frame()
    assert int(info["cathle011c_block_cache_hits"]) >= 1, info

    core = ChatGPTN64EmuCore()
    info = core.load_rom_bytes(_make_synthetic_cathle011b_rom(b"THE LEGEND OF ZELDA MAJORA", b"ZV", "n64"), "MajorasMask.n64")
    assert info["rom_type"] == "N64 LITTLE-ENDIAN", info
    assert info["hle_game"] == ZELDA_MM_PROFILE, info
    assert info["cathle011c_valid_n64_rom"] is True, info

    wrapper = N64CythonWrapper()
    wrapper.load_rom_bytes(bytes(_make_synthetic_cathle011b_rom(b"MYSTERY COMMERCIAL", b"ZZ", "z64")), "MysteryUniversal.z64")
    wrapper.boot()
    profile = wrapper.corn_profile()
...
def _print_boot_summary(info: Dict[str, object]) -> None:
    print("cathle0.1.1b boot summary")
    print(f"rom={info.get('rom')} type={info.get('rom_type')} title={info.get('title')}")
    print(f"booted={info.get('booted')} status={info.get('boot')}")
    print(f"profile={info.get('hle_game')} display={info.get('cathle011b_profile_display')}")
    print(f"recompiler={info.get('cathle011b_recompiler_ready')} blocks={info.get('cathle011b_compiled_blocks')} ops={info.get('cathle011b_compiled_ops')} hash={info.get('cathle011b_recompile_hash')}")
    print(f"frame={info.get('frame')} fps={info.get('target_fps')} mode={info.get('hle_video_mode')} commercial={info.get('cathle011b_commercial_level')}")
    print(f"pc=0x{int(info.get('pc', 0)):08X} op=0x{int(info.get('last_opcode', 0)):08X} decode={info.get('last_decode')}")
...
def main() -> None:
    if "--selftest" in sys.argv:
        _selftest()
        print("cathle0.1.1b selftest ok")
        return
    if "--boot" in sys.argv:
        idx = sys.argv.index("--boot")
        if idx + 1 >= len(sys.argv):
            raise SystemExit("usage: python3.14 cathle0.1.1b.py --boot path/to/rom.z64 [--frames 60]")
        frames = 60
