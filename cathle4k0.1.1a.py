...
"""
n64emu4k.py
N64Emu4K 0.1 - single-file clean-room N64 emulator core + Tkinter GUI + universal SM64 HLE booter.
...
- Universal SM64-family HLE4K ROM booter plus Corn-style no-plugin fast paths.

This file intentionally does not copy Corn, Project64, UltraHLE, Nintendo, or
commercial emulator source. It is a clean-room, educational emulator-core scaffold with an SM64-family HLE boot path.
"""
...
APP_NAME = "ac's N64Emu4K 0.1 - Universal SM64 HLE"
ENGINE_FILE = "n64emu4k.py"
...
CLEANROOM_PROFILE = "UNIVERSAL_SM64_HLE4K_NO_PLUGIN_CORE"
...
class ChatGPTN64EmuGUI:
    ...
    def build_ui(self) -> None:
        ...
        tk.Label(
            toolbar,
            text=" N64Emu4K 0.1 | python3.14 | Corn-style HLE | files off",
            bg=BG,
            fg=BLUE,
            font=("Arial", 8, "bold"),
    ...
    def render_mario_kart_hle(self, s: Dict[str, object]) -> None:
        ...
        self.draw_center_text(15, "N64Emu4K 0.1", GREEN, 9)
        if progress < 40:
...
def _corn_render_corn_hle(self: "ChatGPTN64EmuGUI", s: Dict[str, object]) -> None:
    ...
    if short == "SM64":
        ...
        self.screen.create_oval(kart_x + 2, 125, kart_x + 10, 133, fill=BLACK, outline=BLACK)
        self.screen.create_oval(kart_x + 20, 125, kart_x + 28, 133, fill=BLACK, outline=BLACK)
    self.draw_center_text(14, "N64Emu4K 0.1", GREEN, 9)
...
def main() -> None:
    if "--selftest" in sys.argv:
        _selftest()
        print("n64emu4k selftest ok")
        return
    if "--boot" in sys.argv:
        idx = sys.argv.index("--boot")
        if idx + 1 >= len(sys.argv):
            raise SystemExit("usage: python3.14 n64emu4k.py --boot path/to/rom.z64 [--frames 60]")
        frames = 60
