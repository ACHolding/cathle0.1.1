...
"""
cathle640.1.py
CatHLE64 0.1 - single-file clean-room N64 emulator core + Tkinter GUI + Corn public-feature HLE layer.
...
def main() -> None:
    if "--selftest" in sys.argv:
        _selftest()
        print("cathle640.1 selftest ok")
        return
    if "--boot" in sys.argv:
        idx = sys.argv.index("--boot")
        if idx + 1 >= len(sys.argv):
            raise SystemExit("usage: python3.14 cathle640.1.py --boot path/to/rom.z64 [--frames 60]")
        frames = 60
